...
def list_dispatches_for_authority(authority_id):
    ...
    # Fetch recent dispatch records for the given authority
    dispatches = list(
        mongo.db.dispatches
    ...
    for d in dispatches:
        ...
        # Build combined dispatch and victim response object
        results.append({
            "_id": str(d["_id"]),
            "dispatch": {
                "_id": str(d["_id"]),
...
# Receive accept or reject response for a dispatch
...
def respond_to_dispatch(dispatch_id):
    ...
    try:
        ...
        # Update dispatch response using escalation service
